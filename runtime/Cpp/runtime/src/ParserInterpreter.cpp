...
void ParserInterpreter::visitState(atn::ATNState *p) {
  ...
  DecisionState *const decisionState = antlr_cast<DecisionState *>(p);
  ...
  switch (transition->getSerializationType()) {
    case atn::Transition::EPSILON:
      if (p->getStateType() == ATNState::STAR_LOOP_ENTRY &&
        antlr_cast<StarLoopEntryState *>(p)->isPrecedenceDecision &&
        antlr_cast<LoopEndState *>(transition->target) == nullptr) {
