...
ATN ATNDeserializer::deserialize(const std::vector<uint16_t>& input) {
  ...
  for (size_t i = 0; i < nstates; i++) {
    ...
    } else if (antlr_cast<BlockStartState *>(s) != nullptr) {
  ...
  for (ATNState *state : atn.states) {
    RuleStopState *const stopState = antlr_cast<RuleStopState *>(state);
  ...
  for (ATNState *state : atn.states) {
    for (size_t i = 0; i < state->transitions.size(); i++) {
      ...
      RuleTransition *const ruleTransition = antlr_cast<RuleTransition *>(t);
  ...
  for (ATNState *state : atn.states) {
    BlockStartState *const startState = antlr_cast<BlockStartState *>(state);
    ...
    PlusLoopbackState *const plusLoopbackState = antlr_cast<PlusLoopbackState *>(state);
    StarLoopbackState *const starLoopbackState = antlr_cast<StarLoopbackState *>(state);
    if (plusLoopbackState != nullptr) {
      for (size_t i = 0; i < plusLoopbackState->transitions.size(); i++) {
        ...
        PlusBlockStartState *const plusBlockStartState = antlr_cast<PlusBlockStartState *>(target);
      ...
      for (size_t i = 0; i < starLoopbackState->transitions.size(); i++) {
        ...
        StarLoopEntryState *const starLoopEntryState = antlr_cast<StarLoopEntryState *>(target);
  ...
  for (size_t i = 1; i <= ndecisions; i++) {
    size_t s = data[p++];
    DecisionState *decState = antlr_cast<DecisionState *>(atn.states[s]);
    if (decState == nullptr)
      throw IllegalStateException();
  ...
  if (atn.grammarType == ATNType::LEXER) {
    if (supportsLexerActions) {
      ...
      for (ATNState *state : atn.states) {
        for (size_t i = 0; i < state->transitions.size(); i++) {
          ActionTransition *const actionTransition = antlr_cast<ActionTransition *>(state->transitions[i]);
    ...
    for (std::vector<RuleStartState*>::size_type i = 0; i < atn.ruleToStartState.size(); i++) {
      ...
      if (atn.ruleToStartState[i]->isLeftRecursiveRule) {
        ...
        for (ATNState *state : atn.states) {
          ...
          if (antlr_cast<StarLoopEntryState *>(state) == nullptr) {
            continue;
          }

          ATNState *maybeLoopEndState = state->transitions[state->transitions.size() - 1]->target;
          if (antlr_cast<LoopEndState *>(maybeLoopEndState) == nullptr) {
            continue;
          }

          if (maybeLoopEndState->epsilonOnlyTransitions && antlr_cast<RuleStopState *>(maybeLoopEndState->transitions[0]->target) != nullptr) {
            endState = state;
            break;
          }
        }
...
void ATNDeserializer::markPrecedenceDecisions(const ATN &atn) {
  for (ATNState *state : atn.states) {
    StarLoopEntryState *const starLoopEntryState = antlr_cast<StarLoopEntryState *>(state);
    if (starLoopEntryState == nullptr) {
      continue;
    }
...
void ATNDeserializer::verifyATN(const ATN &atn) {
  ...
  for (ATNState *state : atn.states) {
    ...
    PlusBlockStartState *const plusBlockStartState = antlr_cast<PlusBlockStartState *>(state);
    if (plusBlockStartState != nullptr) {
    ...
    if (starLoopEntryState != nullptr) {
      ...
      if (antlr_cast<StarBlockStartState *>(starLoopEntryState->transitions[0]->target) != nullptr) {
        checkCondition(antlr_cast<LoopEndState *>(starLoopEntryState->transitions[1]->target) != nullptr);
        checkCondition(!starLoopEntryState->nonGreedy);
      } else if (antlr_cast<LoopEndState *>(starLoopEntryState->transitions[0]->target) != nullptr) {
        checkCondition(antlr_cast<StarBlockStartState *>(starLoopEntryState->transitions[1]->target) != nullptr);
        checkCondition(starLoopEntryState->nonGreedy);
      } else {
        throw IllegalStateException();
      }
    ...
    if (antlr_cast<StarLoopbackState *>(state) != nullptr) {
      checkCondition(state->transitions.size() == 1);
      checkCondition(antlr_cast<StarLoopEntryState *>(state->transitions[0]->target) != nullptr);
    }

    LoopEndState *const loopEndState = antlr_cast<LoopEndState *>(state);
    if (loopEndState != nullptr) {
      checkCondition(loopEndState->loopBackState != nullptr);
    }

    RuleStartState *const ruleStartState = antlr_cast<RuleStartState *>(state);
    if (ruleStartState != nullptr) {
      checkCondition(ruleStartState->stopState != nullptr);
    }

    BlockStartState *const blockStartState = antlr_cast<BlockStartState *>(state);
    if (blockStartState != nullptr) {
      checkCondition(blockStartState->endState != nullptr);
    }

    BlockEndState *const blockEndState = antlr_cast<BlockEndState *>(state);
    if (blockEndState != nullptr) {
      checkCondition(blockEndState->startState != nullptr);
    }

    DecisionState *const decisionState = antlr_cast<DecisionState *>(state);
    if (decisionState != nullptr) {
      checkCondition(decisionState->transitions.size() <= 1 || decisionState->decision >= 0);
    } else {
      checkCondition(state->transitions.size() <= 1 || antlr_cast<RuleStopState *>(state) != nullptr);
    }
  }
}
